...
func aim_at():
	...
	ray.look_at(body.get_target().get_global_position(), Vector3.UP)
	body.rotate_y(deg_to_rad(ray.get_rotation().y * body.get_stats().turn_speed))
	if !ray.is_colliding() :
		return
...
func attack() -> void:
	body.get_weapon().look_at(body.get_target().get_global_position())
	body.get_weapon().attack(["Player","Weapon"])

func physics_process(delta):
	if !body.get_target():
		return
	if body.get_position().distance_to(body.get_target().get_global_position()) < 5:
		return
	navigation.set_max_speed(body.get_stats().move_speed)
	navigation.set_target_position(body.get_target().get_global_position())
	var direction = body.get_position().direction_to(navigation.get_next_path_position())
	body.set_velocity(direction*body.get_stats().move_speed)
	body.move_and_slide()
